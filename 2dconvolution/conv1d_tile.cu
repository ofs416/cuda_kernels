...
__global__ void conv_1d_tile(float *input, float *output, int width, 
...
int main() {
    ...
    // compute convolution with custom kernel
     conv_1d_tile<<<gridDim, blockDim, shared_mem_size>>>(d_input, d_output, width, height, filter_size, false);
